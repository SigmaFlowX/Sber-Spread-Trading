...
@njit()
def run_strategy_fast(sber_price_arr, sberp_price_arr, z_score_arr, a_arr, z_threshold, sl_pct):
    ...
    for i in range(len(sberp_price_arr)):
        ...
        if pos == 0:
            ...
            if pos == 1:
                ...
                pnl = long_pnl + short_pnl - total_fee

                if pnl < -total_pos_size * sl_pct/100 or z_score <= 0:
                    balance += pnl
                    pos = 0
...
def test_strategy_slow(sber_price_arr, sberp_price_arr, z_score_arr, a_arr, z_threshold, sl_pct, timestamps,plot=False):
    fee = 0.008 / 100
    initial_balance = 1000000
    balance = initial_balance
    risk_percent = 10
...
def objective(trial, df):
    ...
    sl_pct = trial.suggest_float('sl_pct', 0, 20)

    sber_price_arr, sberp_price_arr, z_score_arr, a_arr = prepare_data_arrays(df, z_window, spread_window)

    return run_strategy_fast(sber_price_arr, sberp_price_arr, z_score_arr, a_arr, z_threshold, sl_pct)
...
if __name__ == "__main__":
    ...
    for train_start, train_end, test_start, test_end in windows:
        ...
        sharpe,absolute_profit, profit, max_pnl, min_pnl, avg_pnl, win_ratio, total_trades, avg_holding_time, annualized_return, paid_fees = test_strategy_slow(sber_price_arr, sberp_price_arr, z_score_arr, a_arr, z_threshold,sl_pct,timestamps,plot=False)
